...
if (!$_POST['title']) {
    $error.="A Event Title is required <br>";
    
}
if (!$_POST['date']) {
    $error.="A Event Date is required <br>";
...
if (!$_POST['address']) {
    $error.="A Event Address is required <br>";
...
if ($error !="") {
    ...
    $sql = "UPDATE event_table SET event_title ='$title',  address ='$address',  event_date='$date', details='$details' WHERE id = '".$id."'  ";

    if (mysqli_query($conn, $sql)) {
      $_SESSION['update'] = "<p class='alert alert-success'>You have successfully edited Your Event</p>";
  header("Location:viewevents.php");
  exit();
  
} else {
   $_SESSION['updatefail'] = "<p class='alert alert-danger'>There was a Problem editing your Event. Try Again</p>".mysqli_error($conn);
  header("Location: viewevents.php");
  exit();
       ...
       $fetch= mysqli_fetch_array($run);
        ?>
                    <div class="form-group">
                      ...
                      <input type="text" class="form-control" id="exampleInputName1" placeholder="Event Title" name="title" value=" <?php echo $fetch['event_title'];  ?> " />
                    </div>
                    <div class="form-group">
                      <label for="exampleInputEmail3">Event Date</label>
                      <input type="text" class="form-control" id="exampleInputEmail3"  placeholder="Event Date (e.g 20-23,January, 2019)" name="date"  value="<?php echo $fetch['event_date'];  ?>" >
                    </div>
                    <div class="form-group">
                      <label for="exampleInputPassword4">Event Address</label>
                      <input type="text" class="form-control" id="exampleInputPassword4"placeholder="Event Address(e.g PO Box 16 Collins Street Benin City)" name="address"  value="<?php echo $fetch['address'];  ?> " >
                    </div>
                   ...
                   <div class="form-group">
                    ...
                    <div class="form-group">
                     <label for="form23">Event Details</label>
